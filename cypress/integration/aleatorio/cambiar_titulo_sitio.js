...
describe ('login', function(){
    ...
    it ('login', function(){
        ...
        cy.get("div[class='form-group ember-view'] input[type='text']").scrollIntoView().focus().type("{selectall}{backspace}"+faker.random.words(5),{force:true});
        cy.get("div[class='description-container form-group ember-view'] input[type='text']").scrollIntoView().focus().type("{selectall}{backspace}"+faker.random.words(5),{force:true});
        cy.get(".view-actions button.gh-btn-primary").click();
        cy.wait(1000);
        cy.get(".gh-btn-red").should("not.to.exist");
